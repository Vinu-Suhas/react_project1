...
export function  Hollywood(){
    ...
    return(<>
       ...
       <h1  style={{textAlign:"left",textDecoration: "underline",textDecorationColor:"red"}}>Hollywood</h1>
       ...
       {data.data.filter(element=>element.category==='hollywood').map((element,index)=>{
           return(
               ...
               {data.data.filter(element=>element.category==='hollywood').slice(0,2).map((element,index)=>{
                   return(            
                       <div key={element.id} className="contentBox">
