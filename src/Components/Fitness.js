...
export function  Fitness(){
    ...
    return(<>
       ...
       <h1  style={{textAlign:"left",textDecoration: "underline",textDecorationColor:"red"}}>Fitness</h1>
       ...
       {data.data.filter(element=>element.category==='fitness').map((element,index)=>{
           return(
               ...
               {data.data.filter(element=>element.category==='fitness').slice(0,2).map((element,index)=>{
                   return(            
                       <div key={element.id} className="contentBox">
