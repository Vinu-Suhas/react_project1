...
export function  Food(){
    ...
    return(<>
       ...
       <h1  style={{textAlign:"left",textDecoration: "underline",textDecorationColor:"red"}}>Food</h1>
       ...
       {data.data.filter(element=>element.category==='food').map((element,index)=>{
           return(
               ...
               {data.data.filter(element=>element.category==='food').slice(0,2).map((element,index)=>{
                   return(            
                       <div key={element.id} className="contentBox">
